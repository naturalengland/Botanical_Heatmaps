...
#knitr::opts_knit$set(root.dir = 'D:/BSBI_HeatMaps/DataLayers/')
...
surveyMonad <- st_read('heatmap_reports_GB_20211129/heatmap_recording_effort_report_2021_11_29.shapefiles/results/recordingdays1000_VC1-VC112.shp',quiet = TRUE) %>% st_drop_geometry() %>%
...
taxacount <- st_read('heatmap_reports_GB_20211129/heatmap_species_counts_report_2021_11_29.shapefiles/results', quiet=T) %>% st_drop_geometry()
...
```{r stat}
p4 <- ggplot(monad_survey, aes(x=SurveyDays1km_40, y=total_freq)) +
  geom_point() + geom_smooth() +xlim(0,50)

ggplotly(p4)

```

These plots demonstrate a strong relationship between the total number of taxa and the survey days per monad. Plotting a linear model between the two variables we can see that these ...
...
summ <- monad_survey %>% group_by(RGN20NM) %>% summarise(meanFreq=mean(total_freq, na.rm=T), minFreq = min(total_freq,na.rm=T), maxFreq = max(total_freq,na.rm=T))
